...
export const ServiceHeadDashboard: React.FC = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      try {
        setLoading(true);
        console.log('🔍 ServiceHead Dashboard: Fetching department metrics...');
        
        // Fetch service head metrics
        ...
        console.error('❌ ServiceHead Dashboard Error:', err);
    ...
    return (
      <div className="p-8">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
          ...
          <p className="text-red-500 text-sm mt-2">Please ensure you are assigned to a department.</p>
        </div>
      </div>
    );
  }
  ...
  const services_performance = metrics.services_performance || [];
  const department = metrics.department || { title: 'Department' };
  const growth = { orders: 12.5, revenue: 8.3 }; // Placeholder
  ...
  return (
    <>
      {/* Animated Header with Department Name */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 animate-fade-in" style={{animationDelay: '0.1s'}}>
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="px-3 py-1 bg-purple-50 rounded-full">
                <span className="text-xs font-bold text-purple-600">Team</span>
              </div>
            </div>
            <p className="text-gray-500 text-sm font-semibold uppercase tracking-wider mb-2">Team Members</p>
            <p className="text-4xl font-black text-gray-900">{overview.team_members || 0}</p>
            <div className="mt-4 pt-4 border-t border-gray-100">
              <div className="flex items-center gap-2 text-sm text-gray-600">
        ...
        <div className="group relative overflow-hidden rounded-2xl bg-white p-6 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 border border-gray-100">
          ...
          <div className="relative z-10">
            ...
            <p className="text-4xl font-black text-gray-900">{overview.services_count || 0}</p>
            <div className="mt-4 pt-4 border-t border-gray-100">
              <div className="flex items-center gap-2 text-sm text-gray-600">
          ...
          <div className="relative z-10">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-white/5 backdrop-blur-sm rounded-xl hover:bg-white/10 transition-colors">
                ...
                <span className="font-black text-white text-lg">{overview.total_orders > 0 ? formatPrice(Math.round(overview.total_revenue/overview.total_orders)) : '₹0'}</span>
              </div>
              <div className="flex items-center justify-between p-3 bg-white/5 backdrop-blur-sm rounded-xl hover:bg-white/10 transition-colors">
                <span className="text-gray-300 font-medium">Active Clients</span>
                <span className="font-black text-white text-lg">{overview.active_clients}</span>
              </div>
              <div className="flex items-center justify-between p-3 bg-white/5 backdrop-blur-sm rounded-xl hover:bg-white/10 transition-colors">
                <span className="text-gray-300 font-medium">Team Members</span>
                <span className="font-black text-white text-lg">{overview.team_members || 0}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8 animate-fade-in" style={{ animationDelay: '0.3s' }}>
        ...
        <div className="rounded-2xl bg-white p-6 shadow-lg border border-gray-100">
          ...
          <div className="space-y-4">
            {services_performance.slice(0, 5).map((service: any) => (
      ...
      {/* Recent Orders */}
      <div className="rounded-2xl bg-white shadow-xl border border-gray-100 overflow-hidden animate-fade-in" style={{ animationDelay: '0.4s' }}>
